...
generateDataExtended <- function(n, seed) {
  set.seed(seed)
  Age_years <- rnorm(n, 65, 5)
  Sodium_time0 <- rnorm(n) # Simulated sodium intake at time 0
  SBP_time1 <- 1.05 * Sodium_time0 + rnorm(n) # Simulated SBP at time 1
  Proteinuria_time2 <- 2.00 * SBP_time1 + 2.80 * Sodium_time0 + rnorm(n) # Simulated proteinuria at time 2
  Sodium_gr <- Age_years / 18 + rnorm(n)
  sbp_in_mmHg <- 1.05 * Sodium_gr + 2.00 * Age_years + rnorm(n)
  hypertension <- ifelse(sbp_in_mmHg >= 140, 1, 0)
  data.frame(sbp_in_mmHg, hypertension, Sodium_gr, Age_years, Proteinuria_time2, SBP_time1, Sodium_time0)
}

# Step 4: Adapted Regression Analyses
ObsDataExtended <- generateDataExtended(n = 1000, seed = 777)

fit6 <- lm(sbp_in_mmHg ~ Sodium_gr + Age_years + Proteinuria_time2, data = ObsDataExtended)

# Step 5: Visualizations and Comparisons
# Adapted Model Visualization
m6 <- visreg(fit6, "Sodium_gr", gg = TRUE, xlab = "Sodium at Time 3 (gr)", ylab = "SBP at Time 4 (mmHg)",
             line = list(col = "red"),
             points = list(size = 2, pch = 1, col = "black"), bty = "n") + 
  theme_classic() +
  ggtitle("Adapted Model with Early Proteinuria")
